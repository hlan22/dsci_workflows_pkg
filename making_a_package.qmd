...
If you're working in RStudio, you should find yourself in a new instance of RStudio, opened into your new package (and Project).
...
| The `R/` directory | The "business end" of your package. It will soon contain `.R` files with function definitions. |
| `*.Rproj`  | The file that makes this directory an RStudio Project. Even if you don't use RStudio, this file is harmless. |

## `use_git()`

This directory is an R source package and an RStudio Project.
...
The only thing that has changed is the creation of a `.git` directory, which is hidden in most contexts, including the RStudio file browser.
...
You can do so manually by quitting, then relaunching RStudio by double clicking on your project file.
Now, in addition to package development support, in RStudio you have access to a basic Git client in the *Git* tab of the *Environment/History/Build* pane.


## Writing functions with `use_r()`
...
How do we test drive our function though?

For package development, devtools offers a more robust approach.

Call `load_all()` to make the functions you made available for experimentation.

```r
load_all()
```
Note that `load_all()` has made the functions available, although it does not exist in the global environment.
...
We have informal, empirical evidence that our functions work.
But how can we be sure that all the moving parts of the package still work?
...
We still need to trigger the conversion of this new roxygen comment into `man/function-name.Rd` with `document()`:
...
You'll see a message like "Rendering development documentation for `function-name`", which reminds that you are basically previewing draft documentation.
...
For many of you, if `R/function-name.R` is the active file in RStudio, you can just call `use_test()`.
However, since this book is built non-interactively, we must provide the basename explicitly:

```{r test-function-name, eval = create}
...
test_path <- path("tests", "testthat", "test-function-name.R")
```

```{cat function-name-test-write, eval = create, class.source = "r"}
#| engine.opts = list(file = test_path)
test_that("function-name() splits a string", {
  expect_equal(function-name("a,b,c", split = ","), c("a", "b", "c"))
})
```

This tests that `function-name()` gives the expected result when splitting a string.

```{r commit-function-name-test, eval = create, include = debug}
git_add(test_path)
git_commit("Test function-name()")
```
...
```{r}
rename_files("function-name", "str_split_one")
```
...
git_add(c(
  "NAMESPACE",
  path("man", c("str_split_one.Rd", "function-name.Rd")),
  path("R", c("str_split_one.R", "function-name.R")),
  path("tests", "testthat", c("test-str_split_one.R", "test-function-name.R"))
))
